...
export default function HistoryScreen({ sessions, onLoad, onDelete, onBack, onLogoClick, initialProject }) {
    ...
    return (
      <div className="min-h-screen bg-gray-50">
        ...
        <div className="max-w-lg mx-auto px-6 py-8">
          <h1 className="text-xl font-semibold text-gray-800 mb-6">Projects</h1>
          {projectNames.length === 0 ? (
            <p className="text-gray-400 text-center py-12">No sessions yet</p>
          ) : (
            <div className="space-y-3">
              {projectNames.map(name => {
                const ps = projects[name]
                const totalTags = ps.reduce((sum, s) => sum + (s.tags?.length || 0), 0)
                const latest = new Date(Math.max(...ps.map(s => s.startedAt)))
                return (
                    ...
                    <div className="flex items-start justify-between">
                      <div>
                        ...
                        <p className="text-xs text-gray-400 mt-0.5">
                          {ps.length} session{ps.length !== 1 ? 's' : ''} · {totalTags} tags
                        </p>
                      </div>
      ...
      <TopBar
        ...
        left={
          <div className="flex items-center gap-1.5 text-sm text-gray-400">
            <button onClick={() => setSelectedProject(null)} className="hover:text-gray-700 cursor-pointer">Projects</button>
            <span>›</span>
            <span className="text-gray-600 truncate max-w-[140px]">{selectedProject}</span>
          </div>
        }
      />
      <div className="max-w-lg mx-auto px-6 py-8">
            ...
            <div key={s.id} className="bg-white rounded-xl border border-gray-100 px-4 py-4">
              ...
              {s.tags && s.tags.length > 0 && (
                <div className="flex gap-1.5 flex-wrap mb-3">
                  {Object.entries(s.tags.reduce((acc, t) => {
              ...
              <div className="flex gap-2">
                ...
                <button onClick={() => { if (confirm('Delete this session?')) onDelete(s.id) }}
